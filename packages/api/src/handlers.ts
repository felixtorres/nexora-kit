...
export function createMessageListHandler(deps: HandlerDeps) {
  return async (req: ApiRequest): Promise<ApiResponse> => {
    ...
    // Strip system messages (prompt context, never user-visible).
    // Keep user, assistant, and tool messages — the frontend needs tool
    // messages to reconstruct tool call blocks from ToolResultContent parts.
